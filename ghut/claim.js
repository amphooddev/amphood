/* Claims creator fees from the Pons V2 fee escrow into the amp wallet.
   The amp wallet must be the token's creator fee recipient (set at launch, or via setCreatorFeeRecipient on the factory, which has a timelock).
...
if (process.argv[1] && process.argv[1].endsWith('claim.js')) {
  const who = process.env.amp_ADDRESS; const o = await owed(who); console.log(`owed to ${who}: ${fmt(o.usdg, 6)} USDG, ${fmt(o.eth)} ETH`);
