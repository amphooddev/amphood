/* One epoch of the amp.
   1. Read the amp wallet's USDG (claimed creator fees).
   2. Split: holders / buyback / keep.
   3. Buy amphood with the holders' share, in slices, each slice timed by the brain clock.
   4. Snapshot holders and drop the amphood pro-rata. One transfer per holder.
   5. Write the ledger the page reads.
...
const w = DRY ? { account: { address: process.env.amp_ADDRESS || '0x0000000000000000000000000000000000000000' } } : wallet();
const amp = w.account.address;
const USDG = getAddress(CFG.quote.address), FLY = getAddress(CFG.flybrain.address), MAG = CFG.AMPHOOD.address;
...
const usdg = await balance(USDG, amp);
console.log(`epoch ${epochId} · amp ${amp} · USDG ${fmt(usdg, 6)}`);
if (usdg < 1_000_000n) { console.log('less than 1 USDG in the amp. claim creator fees on Pons first.'); { process.exitCode = 0; await new Promise(r => setTimeout(r, 300)); throw new ExitSignal(); }; }
...
console.log(`plan: ${fmt(toHolders, 6)} USDG → FLYBRAIN for holders, ${fmt(toBuyback, 6)} USDG → AMPHOOD burn, ${fmt(usdg - toHolders - toBuyback, 6)} USDG kept`);
...
for (let i = 0; i < slices; i++) {
  ...
  const got = await balance(FLY, amp) - flyBought; flyBought = await balance(FLY, amp);
...
// 4. buyback and burn AMPHOOD (only after launch, when the pool is known)
if (MAG && !MAG.startsWith('0x0000') && CFG.AMPHOOD.poolFee !== undefined && toBuyback > 0n) {
  const magKey = poolKey(MAG, USDG, CFG.AMPHOOD.poolFee, CFG.AMPHOOD.poolTickSpacing, CFG.AMPHOOD.hooks || '0x0000000000000000000000000000000000000000');
  const q = await quote(magKey, USDG, toBuyback); console.log(`buyback: ${fmt(toBuyback, 6)} USDG → ~${fmt(q)} AMPHOOD, then burn`);
  if (!DRY) { const before = await balance(MAG, amp); const r = await swap(w, magKey, USDG, toBuyback, CFG.slippageBps); const got = await balance(MAG, amp) - before; const h = await w.client.writeContract({ address: getAddress(MAG), abi: ERC20, functionName: 'transfer', args: ['0x000000000000000000000000000000000000dEaD', got] }); await pub.waitForTransactionReceipt({ hash: h }); e.burn = { usdg: toBuyback.toString(), AMPHOOD: got.toString(), swapTx: r.hash, burnTx: h }; ledger.totals.AMPHOODBurned = (BigInt(ledger.totals.AMPHOODBurned) + got).toString(); writeLedger(ledger); console.log('  burned', fmt(got), 'AMPHOOD', h); }
} else console.log('buyback skipped: AMPHOOD pool not configured yet');

// 5. snapshot and drop
if (!MAG || MAG.startsWith('0x0000')) { console.log('no AMPHOOD address yet: FLYBRAIN stays in the amp until launch.'); e.done = DRY ? false : true; if (!DRY) writeLedger(ledger); { process.exitCode = 0; await new Promise(r => setTimeout(r, 300)); throw new ExitSignal(); }; }
const snap = await snapshot();
const pot = DRY ? await quote(flyKey, USDG, toHolders) : await balance(FLY, amp);
const eligTotal = BigInt(snap.eligibleTotal);
console.log(`drop: ${fmt(pot)} FLYBRAIN to ${snap.eligible} holders (pro-rata over ${fmt(eligTotal)} AMPHOOD)`);
const drops = [];
