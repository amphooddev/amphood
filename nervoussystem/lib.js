...
export function wallet() {
  const pk = process.env.amp_PRIVATE_KEY;
  if (!pk) throw new Error('amp_PRIVATE_KEY missing. Copy .env.example to .env and put the amp wallet key there.');
...
export function readLedger() { try { return JSON.parse(fs.readFileSync(ledgerPath(), 'utf8')); } catch { return { epochs: [], buys: [], drops: [], totals: { usdgClaimed: '0', flyBought: '0', flyDropped: '0', amphoodBurned: '0', recipients: 0 } }; } }
