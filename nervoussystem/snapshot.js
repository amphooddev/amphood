/* Holder snapshot: replays every Transfer of amphood from launch to now and keeps the balances.
...
export async function snapshot(token = CFG.amphood.address, fromBlock = CFG.amphood.launchBlock || 0) {
  ...
  console.log();
  const excl = new Set(CFG.excludeAddresses.map(a => a.toLowerCase())); if (CFG.amphood.poolAddress) excl.add(CFG.amphood.poolAddress.toLowerCase());
  const min = parseUnits(CFG.amphood.totalSupply, CFG.amphood.decimals) * BigInt(Math.round(CFG.minHoldFractionOfSupply * 1e8)) / 100_000_000n;
